...
class SentimentAnalysis(AutomataPlugin):
    ...
    async def sentiment(self, ctx, *, argument=None):
        ...
        has_argument = False if argument is None else True
        has_message_reference = False if ctx.message.reference is None else True

        if not has_argument and not has_message_reference:
        ...
        elif not has_argument and has_message_reference:
